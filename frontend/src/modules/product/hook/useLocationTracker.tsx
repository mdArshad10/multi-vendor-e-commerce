...
export const useLocationTracker = () => {
    const [location, setLocation] = useState<{ contry: string, city: string } | null>(getLocationStore());
    ...
    const fetchLocation = async () => {
        try {
            ...
            localStorage.setItem(LOCATION_STORAGE_KEY, JSON.stringify({ contry: data.country, city: data.city, expireTime: Date.now() }));
        } catch (error) {
            console.log(error);
        }
    }
